...
generate-media-props fail_on_diff="true":
    #!/bin/bash
    set -e
    python openverse_catalog/utilities/media_props_gen/media_props_generation.py \&\& chmod 666 openverse_catalog/utilities/media_props_gen/media_properties.md
    # Move the file to the top level, since that level is not mounted into the container
    mv openverse_catalog/utilities/media_props_gen/media_properties.md media_properties.md
    echo -n "Running linting..."
    # Linting step afterwards is necessary since the generated output differs greatly from what prettier expects
    just lint &>/dev/null || true
    echo "Done!"
    if {{ fail_on_diff }}; then
      set +e
      git diff --exit-code media_properties.md
      if [ $? -ne 0 ]; then
