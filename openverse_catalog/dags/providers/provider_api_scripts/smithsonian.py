...
DEFAULT_PARAMS = {"api_key": API_KEY, "rows": LIMIT}
RETRIES = 3
...
def gather_samples(
    units_endpoint=UNITS_ENDPOINT, default_query_params=None, target_dir="/tmp"
):
    """
...
def _gather_unit_sample(unit, sample_dir, retries=RETRIES, endpoint=SEARCH_ENDPOINT):
    """
    This function is for gathering test data only, and is untested.
    """
    logger.info(f"gathering sample for unit {unit}")
    for hash_prefix in [None, "a", "aa", "aaa", "aaaa", "aaaaa"]:
        query_params = _build_query_params(0, hash_prefix=hash_prefix, unit_code=unit)
        response_json = delayed_requester.get_response_json(
            endpoint, retries=retries, query_params=query_params
        )
        if response_json is None:
            logger.warning(f"response_json is NoneType for {unit}")
            break
        elif response_json["response"]["rowCount"] == 0:
            logger.info(f"No rows found for unit_code {unit} with hash {hash_prefix}")
            break
        elif response_json["response"]["rowCount"] > 10000:
            logger.info(f'Too many rows:  {response_json["response"]["rowCount"]}')
        else:
...
def _process_hash_prefix(
    hash_prefix, endpoint=SEARCH_ENDPOINT, limit=LIMIT, retries=RETRIES
):
...
def _build_query_params(
    row_offset, hash_prefix=None, default_params=None, unit_code=None
):
...
def _get_creator(row, creator_types=None):
    ...
    ordered_freetext_creator_objects = sorted(
        [
            i
            for i in _check_type(freetext.get("name"), list)
            if type(i) == dict
    ...
    indexed_structured_creator_generator = (
        i["content"]
        for i in _check_type(indexed_structured.get("name"), list)
        if type(i) == dict
    ...
    if creator is None:
        logger.debug(f"No creator found in indexed_structured:  {indexed_structured}")
    return creator
...
def _get_descriptive_non_repeating_dict(row):
    ...
    return _check_type(_get_content_dict(row).get("descriptiveNonRepeating"), dict)
