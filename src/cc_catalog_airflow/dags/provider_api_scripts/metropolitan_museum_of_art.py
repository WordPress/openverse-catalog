...
def main(date_=None):
    ...
    date_:  Date String in the form YYYY-MM-DD.  This is the date for
           which running the script will pull data.
    """

    logger.info(f'Begin: Met Museum API requests for date: {date_}')

    fetch_the_object_id = _get_object_ids(date_)
...
def _create_meta_data(object_json):
    meta_data = {
        'accession_number': object_json.get('accessionNumber', None),
        'classification': object_json.get('classification', None),
        'culture': object_json.get('culture', None),
        'date': object_json.get('objectDate', None),
        'medium': object_json.get('medium', None),
        'credit_line': object_json.get('creditLine', None)
    }
