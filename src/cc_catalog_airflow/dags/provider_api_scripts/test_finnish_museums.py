...
def test_get_object_list_with_none_response():
    with patch.object(
        fm.delayed_requester, "get_response_json", return_value=None
    ) as mock_get:
...
def test_build_query_param_default():
    ...
    actual_param_made = fm._build_params(building="0/Museovirasto/")
    expected_param = {
...
def test_build_query_param_given():
    ...
    page = 2
    actual_param_made = fm._build_params(building=building, page=page)
    expected_param = {
        "filter[]": ['format:"0/Image/"', 'building:"0/Museovirasto/"'],
        "limit": 100,
        "page": 2,
...
def test_get_object_list_from_json_returns_expected_output():
    ...
    actual_items_list = fm._get_object_list_from_json(json_resp)
