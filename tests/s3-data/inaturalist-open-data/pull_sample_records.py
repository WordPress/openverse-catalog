"""
This is really just a one time thing I used to generate the sample files, once I had identified
some photo IDs that I thought were worth using for testing. I'm not sure it actually belongs in 
the on-going code base at all, and if it does, it needs a bunch of clean-up and a better location
...
def pull_sample_records(file_name, file_path, id_name, id_list):
    ...
    sample_records = []
    print('Starting to read',working_input_file,'at',dt.now().strftime("%d/%m/%Y %H:%M:%S"))
    with gzip.open(working_input_file, 'rt') as in_file:
        ...
        for record in records:
            ...
            if lines_read % 10**7 == 0:
                print('Read line number',lines_read,'at',dt.now().strftime("%d/%m/%Y %H:%M:%S"))
...
def get_sample_id_list(sample_file, joined_on):
    ...
    sample_file:    from a table that you have already drawn sample records
                    a file object so that you can handle if it's compressed or not, set up to read text
...
if __name__ == '__main__':
    ...
    # PHOTOS
    photo_ids = [str(i) for i in [20314159, 10314159, 30314159, 40314159, 60314159, 80314159, 
        90314159, 110314159, 120314159, 130314159, 150314159, 160314159, 170314159, 
        191019692, 191018903, 191018870, 191016506, 191015547, 191015484, 191012628, 
        191001500, 190995656, 190995604, 191028942, 191028931, 191024617]]
    ...
    pull_sample_records('observations.csv.gz', 'full_june_2022', 'observation_uuid', sample_observations)

    # ASSOCIATED OBSERVERS
    with gzip.open('photos.csv.gz', 'rt') as photo_output:
        sample_observers = get_sample_id_list(photo_output, 'observer_id')
    pull_sample_records('observers.csv.gz', 'full_june_2022', 'observer_id', sample_observers)
    ...
    pull_sample_records('taxa.csv.gz', 'full_june_2022', 'taxon_id', list(sample_taxa_with_ancestors))

    # PRINT RESULTS
    for f in ['photos.csv.gz', 'observations.csv.gz', 'observers.csv.gz', 'taxa.csv.gz']:
        print("="*80)
        print('====> Displaying',f)
